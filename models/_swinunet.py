...
# https://github.com/YunusAbdi/SwinV2-Unet/blob/master/tfswin/model.py#L47
"""
...
from tensorflow.keras.layers import Input, Dense, Conv2D, concatenate, Layer, Embedding, Dropout, LayerNormalization
...
class patch_expanding(tf.keras.layers.Layer):
    ...
    def __init__(self, num_patch, embed_dim, upsample_rate, return_vector=True, name=''):
        ...
        # Sanitize the name to ensure valid scope names
        ...
        self.linear_trans1 = Conv2D(
            ...
            kernel_size=1,
            use_bias=False,
            name=f'{safe_name}_linear_trans1' if safe_name else 'linear_trans1'
        )
        ...
        self.linear_trans2 = Conv2D(
            ...
            kernel_size=1,
            use_bias=False,
            name=f'{safe_name}_linear_trans2' if safe_name else 'linear_trans2'
        )
    ...
    def call(self, x):
        ...
        x = tf.nn.depth_to_space(x,
                                 self.upsample_rate,
                                 data_format='NHWC',
                                 name=f'{self.prefix}_d_to_space' if self.prefix else 'd_to_space')
...
class WindowAttention(tf.keras.layers.Layer):
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0, proj_drop=0., name=''):
        ...
        # Calculate num_relative_distance
    ...
    def build(self, input_shape):
        ...
        #self.relative_position_bias_table = self.add_weight(name='{}_attn_pos'.format(self.prefix),
        #                                                    shape=(num_window_elements, self.num_heads),
        #                                                    initializer=tf.initializers.Zeros(),
        #                                                    trainable=True)

        # Modified initializer that accepts dtype
        self.relative_position_bias_table = self.add_weight(
            shape=(self.num_relative_distance, self.num_heads),
            #initializer=lambda shape, dtype: tf.zeros(shape, dtype=dtype),
            initializer=tf.keras.initializers.TruncatedNormal(stddev=0.02),
            trainable=True,
            name='{}_relative_position_bias_table'.format(self.name))
        ...
        # Convert to the tf variable using add_weight instead of tf.Variable
        self.relative_position_index = self.add_weight(
            name='{}_attn_pos_ind'.format(self.prefix),
            shape=relative_position_index.shape,
            #initializer=lambda _: tf.convert_to_tensor(relative_position_index, dtype=tf.int32),
    ...
    def call(self, x, mask=None):
        ...
        # Reshape q, k, v to match window size
        window_size = self.window_size[0] * self.window_size[1]
        q = tf.reshape(q, shape=(-1, self.num_heads, window_size, head_dim))
        k = tf.reshape(k, shape=(-1, self.num_heads, window_size, head_dim))
        v = tf.reshape(v, shape=(-1, self.num_heads, window_size, head_dim))
...
def SwinTransformerBlock(dim, num_patch, num_heads, window_size, shift_size, num_mlp, qkv_bias, qk_scale, mlp_drop,
                         attn_drop, proj_drop, drop_path_prob, name):
    # Modified initialization
    def _init_weights(shape, dtype=None):
        return tf.random.truncated_normal(shape, stddev=0.02)

    # Use the modified initializer
    return Sequential([
        LayerNormalization(epsilon=1e-5, name='{}_norm1'.format(name)),
        WindowAttention(dim, window_size=(window_size, window_size), num_heads=num_heads,
                        qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=proj_drop,
                        #kernel_initializer=_init_weights,
                        name='{}_attn'.format(name)),
...
def swin_unet(input_shape, n_labels, shift_window=True, name='swin_unet'):
    """
    ...
    -------------------------------------------------------------------------------
    """
    IN = keras.layers.Input(shape=input_shape, dtype=tf.float32)

    filter_num_begin = 512     # number of channels in the first downsampling block; it is also the number of embedded dimensions
    ...
    # Compute the number be patches to be embedded
    input_size = IN.shape[1:]
    ...
    # Upsampling begins atthe deepest available tensor
    X = X_skip[0]
...
# The last expanding layer; it produces full-size feature maps based on the patch size
    ...
    model = tf.keras.models.Model(inputs=IN, outputs=OUT)#, name="Swin UNet")
    return model
