...
Refcode:
https://github.com/xuebinqin/U-2-Net/blob/master/u2net.py
https://github.com/chjort/u2net/tree/master
"""
...
class REBNCONV(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        x = self.bn_s1(x, training=training)
        x = tf.nn.relu(x)
        return x
...
class RSU_N(tf.keras.layers.Layer):
    ...
    def __init__(self, n, filters=12, filters_out=3, name=None):
        ...
        self.upsamples = [kl.UpSampling2D(size=[2, 2], interpolation=sample_interpolation) for _ in range(self.n - 2)]
...
def U2Net(input_shape, n_labels=1):
    inputs = kl.Input(input_shape, dtype=tf.float32)
